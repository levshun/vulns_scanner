...
class InfoGetter:
    ...
    def print_command_execution(self):
        if self.terminal_command_used:
            logger.info(f'Terminal command {self.OKBLUE}{self.terminal_command_used}{self.ENDC} is executed', end='\n'+'*'*50+'\n')
        else:
            logger.warning('No terminal command was provided', end='\n'+'*'*50+'\n')
    ...
    def execute_sql(self, sql_query):
        ...
        conn.commit()
        conn.close()
        logger.debug('Done with executing sql query')
        return
